...
def dashboard():
	if 'username' in session:
		...
		try:
			...
			return render_template("erro_base_dados.html")
	else:
...
def alertas_history():
	if 'username' in session:
		...
		cursor=cnx.cursor() 
		lista_alertas=[]
		cursor.execute("select reason,temp,humid,data from alertas_history")
		records = cursor.fetchall()
		items=[]
		#print(records)
		for row in records:
			lista_alertas.append(row)
		lista_alertas.reverse()
		for i in range(len(lista_alertas)):
			an_it=dict(id=str(i),reason=str(lista_alertas[i][0]),valor_temp=str(lista_alertas[i][1]),valor_humid=str(lista_alertas[i][2]),data=str(lista_alertas[i][3].split(" ")[0]),hora=str(lista_alertas[i][3].split(" ")[1]))
			items.append(an_it)
		

		return render_template("base.html",pag="alertas_history",items=items)
	
	else:
		flash(render_template("boostrap_notification.html",erro="Não está autorizado para ver esta página!"))
		return redirect(url_for('login'))
	# print((lista_alertas[1][0]))
	# for i in range(len(lista_v_h)):
	#     an_it=dict(id=str(i),reason=str(lista_alertas[i][0]),data=str(lista_d_h[i][0]),hora=str(lista_d_h[i][1]))
	#     items.append(an_it)
